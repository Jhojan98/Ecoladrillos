  ...
  return (
    <div className={`home-page${darkMode ? ' dark' : ''}`}>
      ...
      <section className="cards-section">
        <motion.div className="card" whileHover={{ y: -5, boxShadow: '0 8px 16px rgba(0,0,0,0.15)' }} onClick={() => navigate('/consultar-inventario')}>
          <span className="icon">📦</span>
          <h3>Inventario</h3>
          <p>Consulta y gestiona tu inventario ecológico.</p>
        </motion.div>
        <motion.div className="card" whileHover={{ y: -5, boxShadow: '0 8px 16px rgba(0,0,0,0.15)' }} onClick={() => navigate('/dashboard')}>
          <span className="icon">📊</span>
          <h3>Reportes</h3>
        ...
        >
          ...
          <h3>Escáner</h3>
          <p>Escanea para registrar movimientos.</p>
          {fabOpen === 'escaner' && (
            <div className="escaner-menu">
              <button onClick={() => navigate('/registrar-entrada')}>Registrar Entrada</button>
              <button onClick={() => navigate('/registrar-salida')}>Registrar Salida</button>
            </div>
          )}
        </motion.div>
        <motion.div className="card" whileHover={{ y: -5, boxShadow: '0 8px 16px rgba(0,0,0,0.15)' }} onClick={() => navigate('/registro-material')}>
          <span className="icon">🌱</span>
